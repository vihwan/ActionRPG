...
1. 방향키 입력 시, UDP를 통해 로그가 출력이 되도록 테스트 스크립트 추가
...
## 13-1. (EX) 스크립트 재정비
	...
	- 회사에서 경험하고 익혔던 내용들을 바탕으로 스크립트 정리
	- 유지보수 및 확장성을 고려하여 재정비할 것.
	- 디자인 패턴을 활용하여 로직을 다시 설계해 볼것.
...
2. UI WindowPanel이 열리지 않은 문제 수정
...
1. 포트폴리오 개선 계획
	...
	- 유니티 버전 마이그레이션 (2019.4.30f1 -> 2021.3.23f1)
	- 어떤 작업을 토대로 진행할 것인지에 대해 계획 및 설계
...
1. 스테이지 최적화
	...
	- 실시간 글로벌 조명 사용 금지
	- 오쿨루젼 컬링 적용
	- Project Setting - Quality 범주 설정

	> Very Low로 바꾸니 FPS가 확 오름.
...
1. 전투 밸런스
	...
	- 플레이어의 구르기 후 조작 가능 시간을 앞으로 조금 당김

2. 스테이지 재구성

	- 던전 스테이지 설계중
	- 스테이지 전체를 한번에 Mesh Combine을 하려니 로딩이 오래 걸려 작업에 차질이 생기는 현상을 겪는중.
