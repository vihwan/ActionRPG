...
https://rito15.github.io/tabs/categories/

- 에디터 확장
https://m.blog.naver.com/hammerimpact/220769879313
...
2. 퀘스트 달성 / 목표 달성
	...
	- (완료) Travel - 특정 지점 도달	
		특정 지점을 string으로 선언	
		특정지점을 GoalObject로 만들어 게임씬에 배치.
		GoalObject에서 TriggerEnter를 사용해 도달한 지점의 이름을 받아와 비교

	- (완료) Destory - 몬스터, 사물
		Enemy 오브젝트의 이름을 에디터에서 등록
		몬스터가 죽으면 최근에 잡은 몬스터를 등록. 
		최근에 잡은 적의 이름을 비교하여 일치하면	 카운트 증가
	...
	- (완료) Collect - 수집
		특정 Item을 에디터에 등록
		등록한 ItemType에 따라 검색하는 인벤토리가 달라짐
		해당 아이템을 찾게되면 그 수량을 받아와 currentProcessCount에 저장

	- ObjectiveTarget을 String으로 만들고 플레이어가 최근에 죽인 몬스터를 가져와 비교하는 방식으로 만들었는데 string으로 비교하는게 너무 비효율적이다. 다른 방법이 없을까?

	> PropertyDrawer나 CustomEditor 등으로 여러 시도를 해봤지만, 결국 잘 안됬다. 시간이 없기 때문에 나중에 다시 시도



3. NPC가 주는 퀘스트

	- NPC 다이얼로그에 Quest를 주는 기능을 추가
	- 퀘스트에 관하여 대화 내용을 설정할 수 있음
	- 퀘스트 대화 내용이 끝마치면, 퀘스트에 관한 내용이 담긴 UI가 출력되고 수락, 거절 버튼에 따라 수행됨

4. 퀘스트 수락

	- 퀘스트가 수락되면, NPC가 가지고 있던 퀘스트를 복사해, PlayerQuestInventory에 저장
	- NPC는 퀘스트를 수락

5. 퀘스트 수락 가능 여부

	- 수락할 수 있는 퀘스트가 있으면, NPC 머리 위에 노란 느낌표
	- 퀘스트 진행중이면 머리 위에 회색 물음표
	- 퀘스트 완료 가능하면 머리 위에 노란 물음표

6. 전체 게임 퀘스트 관리

	- 각 플레이어마다 수락이 가능한 퀘스트, 진행중인 퀘스트, 이미 클리어한 퀘스트를 정리해주는 매니저가 필요하다.
...
2. 캐릭터 시스템 - 레벨업, 경험치 획득

몬스터로부터 경험치 획득
퀘스트로부터 경험치 획득


레벨업에 대한 경험치를 획득하면, 레벨업. 최대 량 이상이 넘어가면 그 수치만큼 보존
